...
* subtask 不创建 executor="both"；多执行主体协同时，executors 使用数组列出参与的 Scene executor_id。
...
当前用于 Refinement 的动作候选来源：
fusion

动作候选序列，其中可能包含 start_time_hint、end_time_hint 和 local_observation：
{{ ctx.stages.fusion.output.executor_timelines }}
...
* subtasks.executors：只能填写参与该 subtask 的 Scene executor_id 数组；不得写 both、方位词或其他未在 Scene 中出现的 executor。
