...
夹爪视角 A 底层交互 JSON 输出（A 只是数据源编号，不代表任何固定空间方位）：

{{ ctx.stages.wrist_view_left.output.gripper_object_interactions }}

夹爪视角 B 底层交互 JSON 输出（B 只是数据源编号，不代表任何固定空间方位）：

{{ ctx.stages.wrist_view_right.output.gripper_object_interactions }}
