...
输入中的subtask、start_time 和 end_time 均为金标准。不得修改、重写、拆分、合并、增删或重新定位子任务。视频仅用于补充动作前后状态。
...
7. 无法从视频可靠确认时，使用保守表述 “Unable to determine reliably.”，不得猜测。
...
* effect 描述是否发生新的任务相关变化；没有变化时写 “No additional task-relevant state change occurs.”
...
所有描述使用英文。只输出合法 JSON，不输出解释或额外字段。
