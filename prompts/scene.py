...
1. 主输入视角是 Scene 阶段的主要依据。主视角中物体类别、实例数量、空间关系或交互对象清楚可见时，以主视角为准，不得用夹爪视角结果覆盖主视角观察。

2. 只有在主视角证据不足时，才使用夹爪视角作为局部辅助证据。典型情况包括：

   * 机械臂末端、夹爪或接触区域在主视角中被遮挡；
   * 主视角存在视角盲区，无法看清夹爪是否接触、夹住、释放或约束物体；
   * 主视角只能看到物体整体移动，但无法确认被哪一个末端执行器直接作用；
   * 主视角中多个相邻同类物体难以区分，而夹爪视角提供了更清楚的局部颜色、形状、材质、边缘或结构特征；
   * 需要确认局部开合、插入、连接、容纳、卡住、滑移、脱落等细节，而这些细节在主视角中不可见或不清楚。

3. 使用夹爪视角时，只把它作为局部证据来辅助判断物体身份、交互对象或接触细节。不得仅凭夹爪视角中的物体名称新增全局 object_id，也不得仅凭夹爪视角中的局部物体名称替换主视角中更清楚的物体类别。

4. 夹爪视角通常存在运动晃动、视野狭窄、严重遮挡、近距离模糊、物体只显示局部、背景快速变化等问题。若夹爪视角证据不清楚、与主视角冲突或无法唯一对应主视角物体，应优先保留主视角判断，并在 uncertainties 中说明夹爪辅助证据的局限。

5. 当夹爪视角与主视角互补时，应在 description、interaction_reason 或 uncertainties 中体现对应关系的证据，例如“主视角中 object_1 被末端遮挡，夹爪视角 A 在同一时间附近显示夹爪与局部黑色矩形边缘接触，因此辅助确认 object_1 是交互对象”。

6. 凡是 objects 或 interaction_objects 的判定使用了夹爪视角辅助证据，必须在对应 objects.description 或 interaction_objects.interaction_reason 中明确说明：

   * 使用了夹爪视角 A、夹爪视角 B，或两者共同作为辅助证据；
   * 主视角中哪一部分证据不足，例如遮挡、盲区、物体局部细节不清、接触关系不可见或同类物体难以区分；
   * 夹爪视角具体补充了什么视觉证据，例如局部颜色/形状/边缘、夹爪接触、夹住、释放、约束、插入、连接、滑移或脱落；
   * 若未使用夹爪视角辅助证据，则不需要额外说明。
...
"executors": [
{
"executor_id": " base | arm_1 | arm_2 | human | unknown",executor_id
...
"description": "基于 best_view_time 对应主视角画面，描述物体本身的颜色、形状、大小、稳定结构和实例区分特征；如果该物体类别、身份或区分特征使用了夹爪视角辅助证据，必须说明使用了夹爪视角 A/B 的哪类局部证据以及主视角为何不足"
}
],
"interaction_objects": [
{
...
"interaction_reason": "以当前 object_id 为主语，仅描述物体自身的位置、姿态、结构或开合状态变化，或其与其他 object_id 之间接触、分离、容纳、放置、固定、约束、插入或连接关系的变化；相关实体只使用 objects 中已有的 object_id；如果该交互物体判定使用了夹爪视角辅助证据，必须说明使用了夹爪视角 A/B 的哪类接触、夹住、释放、约束、插入、连接、滑移或脱落证据以及主视角为何不足"
}
],
"manual_review": {
"required": false,
