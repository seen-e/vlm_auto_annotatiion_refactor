...
根据完整视频和 Scene 阶段结果，提取单机械臂 executor="single" 自身的原子动作序列。
...
* 视频中可能只显示末端执行器或局部连杆。机械臂暂时被遮挡、离开画面或重新进入画面后，仍属于同一个 executor="single"，不得创建新的 executor。

* 同一机械臂在不同时间、不同位置和不同视角中的观测均保持 executor="single"，不得根据画面位置或进入方向改变身份。

* 如果 Scene 阶段未提供 executor="single"，或错误提供 left、right、both、arm_1 等机械臂身份，应在 uncertainties 中说明，不得在 Analysis 阶段自行创建或使用这些机械臂身份。

* 人类、其他机器人、末端工具以及被机械臂抓持后同步运动的物体不得作为 executor="single"。

【独立分析】

* 只分析 executor="single"，关注其末端运动、末端姿态、夹爪状态，以及其与物体之间的距离、接触、分离和同步运动关系。

* actions 仅按该单机械臂自身动作发生顺序排列。

* 不得根据人类、其他机器人或其他运动主体的行为补全 executor="single" 的动作。

【物体判断】

* Scene 阶段的 interaction_objects 仅是候选物体集合，不表示 executor="single" 已经与其中任一物体发生交互。

* object 和 target 只能引用 Scene 阶段已有的 object_id，但不得仅因为某个 object_id 出现在 interaction_objects 中，就认定 executor="single" 操作了该物体。

* object：executor="single" 在该动作中直接接近、对准、接触、约束、移动或施力的单个物体。如果全过程中 executor="single" 和该物体没有产生接触，则二者不应产生联系；接触必须具有清晰的直接视觉证据。对于抓取或夹持，应清晰看到夹爪闭合并约束物体，否则不得认定发生抓取或夹持。
...
* 对接近、对准等准备动作，只有当后续出现可确认的直接接触或稳定抓持，并且从准备动作到接触期间，executor="single" 的末端连续朝向同一物体时，才允许向前关联该 object。
...
1. 先确定当前动作的粗略候选时间区间；
2. 在该时间区间内观察 executor="single" 的末端运动、末端姿态和夹爪状态变化；
3. 观察 executor="single" 与候选物体之间的距离、接触、分离和同步运动关系变化；
4. 观察物体自身状态或物体间关系变化；
...
* local_observation 只描述 start_time_hint 和 end_time_hint 所对应时间区间内，executor="single" 及其附近候选物体的直接可见状态。

* 根据当前区间实际可见的内容，可描述：

  1. executor="single" 的末端位置、运动方向、末端姿态或夹爪状态；
  2. executor="single" 与附近候选物体之间的距离、接触、遮挡、约束或分离关系；
  3. 附近候选物体的位置、姿态、运动或机构状态。

* local_observation 只描述视觉事实，不得直接使用动作词表中的动作名称，不得提前断言发生了某种动作。

* 不得重新描述完整场景，不得描述与 executor="single" 和当前动作无关的背景物体，也不得混入人类、其他机器人或其他运动主体的动作。

* 未观察到的信息不得根据任务指令、常见操作流程或 Scene 阶段结果补充。

【evidence 要求】

* evidence 只描述支持当前 action、object、target 和 executor="single" 归属的直接视觉变化。

* 根据当前动作实际可见的内容，可描述：

  1. executor="single" 的末端运动、姿态或夹爪状态变化；
  2. executor="single" 与 object 之间的距离、接触、约束、同步运动或分离关系；
...
* 除 timeline 的第一个动作外，仅在存在直接可见衔接时，说明当前动作与前一个动作的关系。
...
完整观看视频后，提取 executor="single" 自身的原子动作序列。

仅为单机械臂输出一条独立 timeline，executor 固定为 "single"。只分析该机械臂自身的末端运动、末端姿态、夹爪状态及其与物体的直接关系，不得根据人类、其他机器人或其他运动主体的动作补全该机械臂的动作。

视频中可能只显示末端执行器或局部连杆。机械臂暂时被遮挡、离开画面、重新进入画面，或者在不同视角中处于不同位置时，仍属于同一个 executor="single"，不得创建新的 executor。

夹爪、灵巧手、腕部、末端执行器和局部连杆均属于 executor="single" 的组成部分；末端工具和被抓持物体不得作为独立 executor。

如果 Scene 阶段存在 executor="single"，必须继承该身份。如果 Scene 阶段未提供 "single"，或提供了 left、right、both、arm_1 等机械臂身份，应在 uncertainties 中说明，不得在 Analysis 阶段使用这些身份重新建立机械臂 timeline。

Scene 阶段的 interaction_objects 只是候选物体集合，不代表 executor="single" 已经与其中任一物体发生交互。

其中：

* object 是 executor="single" 在该动作中直接作用的单个物体；
...
对于每个动作，先输出粗略候选时间区间，再观察该区间内 executor="single" 的局部视觉状态：
...
* 当前粗时间区间内 executor="single" 的末端运动、末端姿态和夹爪状态；

* executor="single" 与附近候选物体之间直接可见的距离、接触、遮挡、约束或分离关系；

* 附近候选物体直接可见的位置、姿态、运动或机构状态。

local_observation 只描述视觉事实，不得直接使用动作词表中的动作名称，不得提前断言动作结果，不得描述完整场景、无关背景物体或其他运动主体的动作。

每个动作的 evidence 只描述：

* executor="single" 的直接可见变化；

* executor="single" 与 object 或候选物体之间的直接关系变化；
      ...
      "actions": [
        {
          "start_time_hint": 1.2,
          "end_time_hint": 2.8,
          "local_observation": "当前粗时间区间内 executor=single 及其附近候选物体的直接可见状态",
...
executor_timelines 必须且只能包含一条 executor="single" 的 timeline。
